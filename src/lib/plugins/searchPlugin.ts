...
type SearchMeta = Partial<Pick<SearchState, "query" | "replacement" | "caseSensitive" | "currentIndex">> & {
  clear?: boolean;
};
...
function findMatches(state: EditorState, query: string, caseSensitive: boolean): SearchMatch[] {
  if (!query) return [];
...
function buildState(previous: SearchState, tr: Transaction, state: EditorState): SearchState {
  ...
  const matches = shouldRecompute ? findMatches(state, base.query, base.caseSensitive) : base.matches;
...
function scrollToCurrentMatch(view: EditorView) {
  requestAnimationFrame(() => {
    const currentMatch = view.dom.querySelector<HTMLElement>(".search-match-current");
...
export function setSearchCaseSensitive(view: EditorView, caseSensitive: boolean) {
...
export function replaceCurrentSearchMatch(view: EditorView, replacement: string) {
  const state = getSearchState(view.state);
  ...
  view.dispatch(
    view.state.tr
      ...
      .setMeta(searchPluginKey, { replacement, currentIndex: state.currentIndex })
      .scrollIntoView(),
  );
  scrollToCurrentMatch(view);
}
...
export function replaceAllSearchMatches(view: EditorView, replacement: string) {
  ...
  view.dispatch(tr.setMeta(searchPluginKey, { replacement, currentIndex: 0 }).scrollIntoView());
}
...
export const searchPlugin = $prose(() =>
  new Plugin<SearchState>({
    ...
    state: {
      ...
      apply: (tr, value, _oldState, newState) => buildState(value, tr, newState),
    },
    props: {
      decorations(state) {
        ...
        return DecorationSet.create(
          ...
          searchState.matches.map((match, index) =>
            Decoration.inline(match.from, match.to, {
              class: index === searchState.currentIndex ? "search-match search-match-current" : "search-match",
            }),
          ),
        );
      },
